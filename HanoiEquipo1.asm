...
main:
	...
	add tempCount, N, zero  		# Guardamos el n�mero de addDisks para manipularlo y no alterar la variable original
	
	addDisks: 					# Agregar los discos a la primer columna
		sw tempCount, 0(startTower)		# Agrega el disco a la posici�n 0
	...
	jal ra, hanoi				# Llamo a la funci�n hanoi, guardando en ra el registro
...
hanoi:	
	...
	beq N, condicional, casoBase		# Si N = 1 se va a la funci�n caso base

	# Swap: Hacemos swap de la torre destino y la auxiliar
	addi N, N, -1				# N-1 se vuelve la variable del ciclo para saber en qu� disco vamos
	...
	jal ra, hanoi				# Saltamos a la funci�n Hanoi
	...
	addi startTower, startTower, -4		# Se toma el disco de Inicio que est� m�s arriba
	...
	lw ra, 0(sp)				# Se carga el return adress de la posici�n
...
casoBase:
	...
	addi endTower, endTower, 4 		# Se aumenta el apuntador del destino que se le agreg� un disco
